...
export default class GameConnection {
  ...
  constructor(systems) {
    ...
    this.store = systems.Store; // 亚洲服务器地址(自定义)读取
  }
  ...
  init() {
    ...
    this.region = 'europe';
    // 亚洲服地址可自定义(设置面板「亚洲服务器地址」,Store settings.asiaServer):
    // 空 = localhost(本地 C# 服);可填 host 或 host:port(端口缺省按 localPorts 模式表)。
    const asiaCustom = (this.store && this.store.get('settings', 'asiaServer')) || '';
    this.regionHosts = {
      europe: null,
      america: 'na.astrio.io',
      asia: asiaCustom || 'localhost',
    };
  ...
  _doConnect(serverArg) {
    ...
    if (serverArg.includes('localhost') || serverArg.includes('127.0.0.1')) {
      ...
      if (target) {
        ...
        // 亚洲服(region=asia,本地 C# 服或自定义地址)始终 ws://(无 TLS)。
        const isAsia = this.region === 'asia' && target !== this.host;
        const scheme = isAsia ? 'ws' : (this.view.__ASTRIO_LIVE_WS ? 'wss' : (this.isSecure ? 'wss' : 'ws'));
        // 自定义地址已含端口时不再拼模式端口
        const portPart = isAsia && !target.includes(':') && this.localPorts[serverArg]
          ? ':' + this.localPorts[serverArg] : '';
