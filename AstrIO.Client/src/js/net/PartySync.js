...
export default class PartySync {
  ...
  /** 128 号:指挥官标记点(发送 + 本地画点);wx/wy 缺省取鼠标当前世界坐标。 */
  commander(wx, wy) {
    if (!this.chatSocket.connected) {
      return;
    }
    const x = (wx == null ? this.mouse.canvasX : wx) | 0;
    const y = (wy == null ? this.mouse.canvasY : wy) | 0;
    this.writer.init();
    this.writer.writeUInt8(128);
    this.writer.writeUInt16(x);
    this.writer.writeUInt16(y);
    this.sendPacket(this.writer);
    this.canvas.commanderPoints.add({ x, y, time: this.app.time });
  }
