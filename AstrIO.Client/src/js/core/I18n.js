...
export default class I18n {
  ...
  /**
   * 运行时取词(JS 模块用,与 hstr 属性同一语言包体系)。
   * 用法: i18n.t("touchMenu", "editorTitle")
   * 当前语言缺词时回退 EN,EN 也没有时回退传入的 fallback(缺省键名)。
   */
  t(section, key, fallback) {
    const sel = this.view['lang_' + this.selected];
    const def = this.view['lang_' + this.default];
    return (sel && sel[section] && sel[section][key])
      || (def && def[section] && def[section][key])
      || fallback || key;
  }
